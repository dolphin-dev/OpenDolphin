...
public class KarteEditor extends AbstractChartDocument implements IInfoModel, NChartDocument {
    ...
    public void save() {
        ...
        try {
            // 何も書かれていない時はリターンする
            if (!stateMgr.isDirty()) {
                ClientContext.getBootLogger().debug("not dirty");
                return;
            }
    ...
    private void save1(final SaveParams params) throws DolphinException {
        ...
        ClientContext.getBootLogger().debug("save1 confirmed = " + confirmed);
        ...
        ClientContext.getBootLogger().debug("docInfo.firstConfirmDate = " + docInfo.getFirstConfirmDate());
        ...
        if (!params.isTmpSave()) {
            ...
            if (oldStatus.equals(STATUS_NONE)) {
                ...
                sendClaim = false;
                sendLabtest = false;
                ClientContext.getBootLogger().debug("NONEから確定 : " + sendClaim);

            } else if (oldStatus.equals(STATUS_TMP)) {

                sendClaim = false;
                sendLabtest = false;
                ClientContext.getBootLogger().debug("仮保存から確定 : " + sendClaim);
                ...
                sendClaim = false;
                sendLabtest = false;
                ClientContext.getBootLogger().debug("修正 : " + sendClaim);
            }
            ...
            sendClaim = false;
            sendLabtest = false;
            ClientContext.getBootLogger().debug("仮保存 : " + sendClaim);
        ...
        ClientContext.getBootLogger().debug("docInfo.title = " + docInfo.getTitle());
        ...
        for (ModuleModel mb : moduleBeans) {
            ...
            ClientContext.getBootLogger().debug("stamp size(KB) = " + size);
            totalSize += size;
        }
        ClientContext.getBootLogger().debug("stamp total size(KB) = " + totalSize);
        totalSize = 0;
        ...
        if (imagesimages != null && imagesimages.size() > 0) {
            for (SchemaModel sm : imagesimages) {
                ...
                ClientContext.getBootLogger().debug("schema size(KB) = " + size);
                totalSize += size;
            }
            ClientContext.getBootLogger().debug("total schema size(KB) = " + totalSize);
        }
        ...
        DBTask task = new DBTask<Void, Void>(chart) {
            ...
            protected void succeeded(Void result) {
                ClientContext.getBootLogger().debug("KarteSaveTask succeeded");
    ...
    private void save2(final SaveParams params) throws DolphinException {
        ...
        Date confirmed = saved;
        docInfo.setConfirmDate(confirmed);
        ClientContext.getBootLogger().debug("save2 confirmed = " + docInfo.getConfirmDate());
        ...
        if (progressInfos == null) {
            ...
            ClientContext.getBootLogger().debug("ModuleInfoBean[] progressInfo created");
        ...
        model.clearModules();
        model.clearSchema();
        ClientContext.getBootLogger().debug("model.clearModules(), model.clearSchema()");
        ...
        ModuleModel[] soa = dumper.getModule();
        if (soa != null && soa.length > 0) {
            ClientContext.getBootLogger().debug("soaPane dumped, number of SOA modules = " + soa.length);
            model.addModule(soa);
        } else {
            ClientContext.getBootLogger().debug("soaPane dumped, no module");
        }
        ...
        if (schemas != null && schemas.length > 0) {
            ...
            ClientContext.getBootLogger().debug("schema dumped, number of SchemaModel = " + schemas.length);
        } else {
            ClientContext.getBootLogger().debug("zero schema dumped");
        }
        ...
        if (plan != null && plan.length > 0) {
            ...
            ClientContext.getBootLogger().debug("p dumped, number of p = " + plan.length);
        } else {
            sendClaim = false;
            ClientContext.getBootLogger().debug("p dumped, number of p = 0");
        }
        ...
        // カルテ保存、CLAIM 送信
        //--------------------------------------------------------
        DBTask dbTask = new DBTask<Integer, Void>(this.getContext()) {
            ...
            protected Integer doInBackground() throws Exception {
                ...
                long pvtPK = context.getPatientVisit()!=null ? context.getPatientVisit().getId() : 0L;
                if (sendClaim && pvtPK!=0L) {
                    theState = modify ? 4 : 2;
                }
...
//                }
            ...
            protected void succeeded(Integer result) {
                ...
                // status 変更
                context.setChartState(theState);

                // 印刷
                int copies = params.getPrintCount();
                if (copies > 0) {
                    printPanel2(context.getContext().getPageFormat(), copies, false);
                }
                ...
                // ここで保存したカルテが履歴に表示される
                //--------------------------------
                context.getDocumentHistory().getDocumentHistory();
