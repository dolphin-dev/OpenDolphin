...
public class AreaNetWorkSettingPanel extends AbstractSettingPanel {
    ...
    class NetworkModel {
        ...
        public void populate(ProjectStub stub) {
            ...
            boolean join = Project.getBoolean(Project.JOIN_AREA_NETWORK);   //stub.getJoinAreaNetwork();
            joinAreaNetwork.setSelected(join);
            noJoinAreaNetwork.setSelected(! join);
            
            String val = Project.getString(Project.AREA_NETWORK_NAME);  //stub.getAreaNetworkName();
            if (val != null) {
            ...
            val = Project.getString(Project.AREA_NETWORK_FACILITY_ID);  //stub.getAreaNetworkFacilityId();
            val = val != null ? val : "";
            facilityIdField.setText(val);
            
            val = Project.getString(Project.AREA_NETWORK_FACILITY_ID);  //stub.getAreaNetworkCreatorId();
            val = val != null ? val : "";
        ...
        public void restore(ProjectStub stub) {
            ...
            Project.setBoolean(Project.JOIN_AREA_NETWORK, join);    //stub.setJoinAreaNetwork(join);
            
            NameValuePair pair = (NameValuePair) areaNetworkCombo.getSelectedItem();
            Project.setString(Project.AREA_NETWORK_NAME, pair.getValue());  //stub.setAreaNetworkName(pair.getValue());
            
            String val = facilityIdField.getText().trim();
            if (!val.equals("")) {
                Project.setString(Project.AREA_NETWORK_FACILITY_ID, val);   //stub.setAreaNetworkFacilityId(val);
            }
            
            val = creatorIdField.getText().trim();
            if (!val.equals("")) {
                Project.setString(Project.AREA_NETWORK_CREATOR_ID, val);    //stub.setAreaNetworkCreatorId(val);
            }
        }
    }
