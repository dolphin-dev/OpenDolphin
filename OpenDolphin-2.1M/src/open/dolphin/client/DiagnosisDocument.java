...
public final class DiagnosisDocument extends AbstractChartDocument implements PropertyChangeListener {
    ...
    public void importStampList(final List<ModuleInfoBean> stampList, final int insertRow) {
        ...
        DBTask task = new DBTask<List<StampModel>, Void>(getContext()) {
            ...
            protected void succeeded(List<StampModel> list) {
                ClientContext.getBootLogger().debug("importStampList succeeded");
                if (list != null) {
    ...
    public void save() {
        ...
        ClientContext.getBootLogger().debug("sendDiagnosis = " + sendDiagnosis);

        // continue to save
        Date confirmed = new Date();
        ClientContext.getBootLogger().debug("confirmed = " + confirmed);
        ...
        if (addedDiagnosis != null && addedDiagnosis.size() > 0) {
            ...
            for (RegisteredDiagnosisModel rd : addedDiagnosis) {
                ...
                ClientContext.getBootLogger().debug("added rd = " + rd.getDiagnosis());
                ClientContext.getBootLogger().debug("id = " + rd.getId());
        ...
        if (updatedDiagnosis != null && updatedDiagnosis.size() > 0) {
            ...
            for (RegisteredDiagnosisModel rd : updatedDiagnosis) {
                ...
                ClientContext.getBootLogger().debug("updated rd = " + rd.getDiagnosis());
                ClientContext.getBootLogger().debug("id = " + rd.getId());
    ...
    public void getDiagnosisHistory() {
        ...
        DBTask task = new DBTask<List<RegisteredDiagnosisModel>, Void>(getContext()) {
            ...
            protected List<RegisteredDiagnosisModel> doInBackground() throws Exception {
                ClientContext.getBootLogger().debug("getDiagnosisHistory doInBackground");
                DocumentDelegater ddl = new DocumentDelegater();
            ...
            protected void succeeded(List<RegisteredDiagnosisModel> list) {
                ClientContext.getBootLogger().debug("getDiagnosisHistory succeeded");
                if (list != null) {
                    if (ascend) {
    ...
    public void delete() {
        ...
        DBTask task = new DBTask<Void, Void>(getContext()) {
            ...
            protected Void doInBackground() throws Exception {
                ClientContext.getBootLogger().debug("delete doInBackground");
            ...
            protected void succeeded(Void result) {
                ClientContext.getBootLogger().debug("delete succeeded");
                tableModel.deleteAt(row);
                //setDiagnosisCount(tableModel.getObjectCount());
                // 更新リストにある場合
                // 更新リストから取り除く
...
//    }
    ...
    class DiagnosisPutTask extends DBTask<List<Long>, Void> {
        ...
        protected List<Long> doInBackground() throws Exception {
            ...
            ClientContext.getBootLogger().debug("doInBackground");

            // 更新する
            if (updated != null && updated.size() > 0) {
                ClientContext.getBootLogger().debug("ddl.updateDiagnosis");
            ...
            if (added != null && added.size() > 0) {
                ClientContext.getBootLogger().debug("ddl.putDiagnosis");
                result = ddl.putDiagnosis(added);
                ClientContext.getBootLogger().debug("ddl.putDiagnosis() is NoErr");
                for (int i = 0; i < added.size(); i++) {
                    long pk = result.get(i).longValue();
                    ClientContext.getBootLogger().debug("persist id = " + pk);
            ...
            if (sendClaim && addedDiagnosis != null && addedDiagnosis.size() > 0) {
                ClientContext.getBootLogger().debug("sendClaim Diagnosis");
        ...
        protected void succeeded(List<Long> list) {
            ClientContext.getBootLogger().debug("DiagnosisPutTask succeeded");
            clearDiagnosisList();
        }
    }
