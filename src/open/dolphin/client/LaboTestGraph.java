...
public class LaboTestGraph extends JPanel implements ComponentListener {
	...
	protected Ellipse2D getPlotCircle2D(Point2D point) {
		return new Ellipse2D.Float((float) point.getX() - plotCircleRadius
				/ 2.0F, (float) point.getY() - plotCircleRadius / 2.0F,
	...
	protected void setupRelativeWorld(Graphics2D g2) {
		...
		for (int i = 0; i < data.length; i++) {
			...
			float maxVal = (maxValue[i] - lowerLimit[i])
					/ (upperLimit[i] - lowerLimit[i]);
			float minVal = (minValue[i] - lowerLimit[i])
					/ (upperLimit[i] - lowerLimit[i]);
	...
	protected void plotRelativeValue(Graphics2D g2, FontMetrics fm,
		...
		float xe = getPixelsForSample(xMin) - fm.stringWidth(str)
				- EXAMPLE_FRAME_SPACE;
	...
	private void drawUpperLowerLine(Graphics2D g2, FontMetrics fm) {
		...
		if (unit != null) {
			...
			xp = getPixelsForSample(xMin) - fm.stringWidth(unit)
					- fm.stringWidth((String) yScales[yScales.length - 1])
					- 5.0F;
			yp = getPixelsForValue(yMax) + fm.getDescent();
