...
public class LaboTestGraph extends JPanel implements ComponentListener {
    ...
    Color[] plotColors = new Color[]{
        Color.red, Color.blue, Color.pink, Color.cyan, Color.orange, Color.green,Color.magenta, Color.yellow
    };
    ...
    protected Ellipse2D getPlotCircle2D(Point2D point) {
        return new Ellipse2D.Float( (float)point.getX() - plotCircleRadius / 2.0F,
                                    (float)point.getY() - plotCircleRadius / 2.0F,
    ...
    protected Color getPlotColor(int index) {
        ...
        color = (index >= plotColors.length) ? 
                plotColors[(index % plotColors.length)] : plotColors[index];
    ...
    protected void drawData(Graphics2D g2) {
        ...
        BasicStroke dashStroke = new BasicStroke(valueLineWidth, BasicStroke.CAP_BUTT,BasicStroke.JOIN_MITER,10.0F,yScaleDashPattern,0);
        ...
        for (int i = 0; i < data.length; i++) {
            ...
            if (mode == TT_ABSOLUTE_GRAPH) {
                ...
                plotRelativeValue(g2, fm, lineStroke, dashStroke, i, bValueCircle);
            }
        }
    }
    ...
    protected void setupRelativeWorld(Graphics2D g2) {
        ...
        if (! hasOffset) {
            ...
            //offsetRight = OFFSET_RIGHT + fm.stringWidth(UPPER_LIMIT_STRING) + 2;
    ...
    protected void drawRelativeCoordinate(Graphics2D g2) {
        ...
        // X 軸(サンプル日)の境界を書く
        g2.setStroke(new BasicStroke(scaleLineWidth, BasicStroke.CAP_BUTT,BasicStroke.JOIN_MITER,10.0F,yScaleDashPattern,0));
        for (int j = 1; j < sampleTime.length; j++) {
            
            xp = getPixelsForSampleBound(j); 
            g2.draw(new Line2D.Float(new Point2D.Float(xp, 0.0F),new Point2D.Float(xp, canvasHeight)));
        }
        ...
        //str = "検体採取日";
        //xp = ( getPixelsForSample(xMax) - getPixelsForSample(xMin) - fm.stringWidth(str) ) / 2;
    ...
    protected void plotRelativeValue(Graphics2D g2, FontMetrics fm, BasicStroke lineStroke, BasicStroke dashStroke, int index, boolean circle) {
        ...
        //float ye = getPixelsForValue(UPPER_LIMIT_VALUE) + (float)(index) * EXAMPLE_SPACE;
    ...
    protected Object[] getScale(float max, float min) {
        ...
        for (int i = 0; i < ret.size(); i++) {
            System.out.println((String)ret.get(i));
        }*/
    ...
    protected int getFigureNumber(float value) {
        ...
        if (value >= 1.0F) {
            ...
            /*while ( Math.po(10.0D, keta) * (double)value ) {
    ...
    protected void drawCoordinate(Graphics2D g2) {
        ...
        // Y 軸のスケールを入れる
        //Object[] scale = getScale(maxValue[itemIndex]);
        
        //float maxScale = Float.parseFloat((String)yScales[yScales.length -1]);
        ...
        for (int i = 0; i < yScales.length; i++) {
            ...
            g2.draw(new Line2D.Float(new Point2D.Float(0.0F, yp), new Point2D.Float(canvasWidth, yp) ));
    ...
    protected void plotValue(Graphics2D g2, FontMetrics fm, BasicStroke lineStroke, BasicStroke dashStroke, int index, boolean circle) {
        ...
        //float ye = getPixelsForValue(2.0F * yMax / 3.0F) + (float)(index) * EXAMPLE_SPACE;
        g2.drawString(str, xe, ye);
        xe = xe + fm.stringWidth(str);
