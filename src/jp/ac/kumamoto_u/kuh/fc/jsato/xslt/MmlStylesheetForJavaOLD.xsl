...
<!--
	...
	The author (J.S) made some modifications so that the Java XSLT transformer can work 
...
<!--************************************************************************-->
	<xsl:template match="docInfo">
		<table border="0" width="100%" height="30" cellspacing="2" cellpadding="4">
			<td bgcolor="#6699ff" align="center" valign="middle">
...
<!--************************************************************************-->
	<xsl:template match="mmlLb:TestModule">
		...
		<table border="1" cellspacing="0" cellpadding="4">
	   		<tr bgcolor="#aaaaaa">
				<th>検査項目</th><th>結果値</th><th>単位</th><th>上限値</th><th>下限値</th><th>異常</th>
			</tr>
	...
	<xsl:template match="mmlLb:information">
		<table>
			<tr>
				<td><b>報告状態</b></td>
				<td><xsl:value-of select="mmlLb:reportStatus"/></td>
				<td><b>検査セット</b></td>
				<td><xsl:value-of select="mmlLb:set"/></td>
			</tr>
			<tr>
				<td><b>依頼番号</b></td>
				<td><xsl:value-of select="@mmlLb:registId"/></td>
				<td><b>採取日時</b></td>
				<td><xsl:value-of select="@mmlLb:sampleTime"/></td>
			</tr>
			<tr>
				<td><b>受付日時</b></td>
				<td><xsl:value-of select="@mmlLb:registTime"/></td>
				<td><b>報告日時</b></td>
				<td><xsl:value-of select="@mmlLb:reportTime"/></td>
			</tr>
			<tr>
				<td><b>依頼施設</b></td>
				<td><xsl:value-of select="mmlLb:facility"/></td>
				<td><b>依 頼 者</b></td>
				<td><xsl:value-of select="mmlLb:client"/></td>
			</tr>
<!--
			<tr>
				<td><b>依頼診療科</b></td>
				<td><xsl:value-of select="mmlLb:department"/></td>
				<td><b>依頼病棟</b></td>
				<td><xsl:value-of select="mmlLb:ward"/></td>
			</tr>
-->
			<tr>
				<td><b>検査施設</b></td>
				<td><xsl:value-of select="mmlLb:laboratoryCenter"/></td>
				<td><b>検 査 者</b></td>
				<td><xsl:value-of select="mmlLb:technician"/></td>
			</tr>
		</table>
		<xsl:if test="mmlLb:repMemo">
			<br/>
			<table border="1" cellspacing="0" cellpadding="3">
				<xsl:for-each select="mmlLb:repMemo">
					<tr>
						<th bgcolor="#aaaaaa">報告コメント</th>
						<td><xsl:value-of select='.'/></td>
					</tr>
				</xsl:for-each>
			</table>
		</xsl:if>
		<xsl:if test="mmlLb:repMemoF">
			<br/>
			<table border="1" cellspacing="0" cellpadding="3">
				<tr>
					<th bgcolor="#aaaaaa">フリーコメント</th>
					<td><xsl:value-of select="mmlLb:repMemoF"/></td>
				</tr>
			</table>
		</xsl:if>
	</xsl:template>
	...
	<xsl:template match="mmlLb:specimen">
		<tr bgcolor="#dddddd"><td colspan="6"><b><xsl:value-of select="mmlLb:specimenName"/></b>
			<xsl:if test="mmlAd:zip">（<xsl:value-of select="mmlLb:spcMemo"/>）</xsl:if>
		</td></tr>
	</xsl:template>

	<!--	検査項目のテンプレート		-->
	<xsl:template match="mmlLb:item">
		<td><xsl:value-of select="mmlLb:itemName"/></td>
		<xsl:choose>
			<xsl:when test="mmlLb:numValue">
				<td align="right"><xsl:value-of select="mmlLb:value"/></td>
				<td><xsl:value-of select="mmlLb:unit"/></td>
				<td align="right"><xsl:value-of select="mmlLb:numValue/@mmlLb:up"/></td>
				<td align="right"><xsl:value-of select="mmlLb:numValue/@mmlLb:low"/></td>
				<td><xsl:apply-templates select="mmlLb:numValue"/></td>
			</xsl:when>
			<xsl:otherwise>
				<td><xsl:value-of select="mmlLb:value"/></td>
				<td>　</td>
				<td>　</td>
				<td>　</td>
				<td>　</td>
	...
	<!--	検査結果判定のテンプレート		-->
	<xsl:template match="mmlLb:numValue">
		<xsl:choose>
			<xsl:when test="mmlLb:numValue/@mmlLb:out='H'">
				<font color="ff0000">上限値越え</font>
			</xsl:when>
			<xsl:when test="mmlLb:numValue/@mmlLb:out='L'">
				<font color="0000aa">下限値未満</font>
			</xsl:when>
			<xsl:when test="mmlLb:numValue/@mmlLb:out='N'">基準値内</xsl:when>
			<xsl:otherwise>
				<xsl:value-of select="mmlLb:numValue/@mmlLb:out"/>
			</xsl:otherwise>
		</xsl:choose>
	</xsl:template>
