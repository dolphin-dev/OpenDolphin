			...
			if (atts != null) {
				for (int i=0; i < atts.getLength(); ++i) {
					if ( ((String)atts.getQName(i)).equals("mmlRd:code") ) {
						...
						((mmlRddiagnosis)builder.getElement()).set__mmlRdcode( atts.getValue(i) );
					}
					if ( ((String)atts.getQName(i)).equals("mmlRd:system") ) {
						set__mmlRdsystem( atts.getValue(i) );
						((mmlRddiagnosis)builder.getElement()).set__mmlRdsystem( atts.getValue(i) );
					}
				}
			}
	...
	public boolean buildEnd(String namespaceURI, String localName, String qName, MMLBuilder builder) {
		if (qName.equals("mmlRd:diagnosis") == true) {
			...
			if (parentElement.getQName().equals("mmlRd:RegisteredDiagnosisModule")) {
				((mmlRdRegisteredDiagnosisModule)builder.getParent()).set_diagnosis((mmlRddiagnosis)builder.getElement());
			}
