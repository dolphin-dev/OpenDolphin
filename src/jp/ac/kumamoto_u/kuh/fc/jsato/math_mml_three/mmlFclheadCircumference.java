			...
			if (atts != null) {
				for (int i=0; i < atts.getLength(); ++i) {
					if ( ((String)atts.getQName(i)).equals("mmlFcl:unit") ) {
						...
						((mmlFclheadCircumference)builder.getElement()).set__mmlFclunit( atts.getValue(i) );
					}
				}
			}
	...
	public boolean buildEnd(String namespaceURI, String localName, String qName, MMLBuilder builder) {
		if (qName.equals("mmlFcl:headCircumference") == true) {
			...
			if (parentElement.getQName().equals("mmlFcl:birthInfo")) {
				((mmlFclbirthInfo)builder.getParent()).set_headCircumference((mmlFclheadCircumference)builder.getElement());
			}
