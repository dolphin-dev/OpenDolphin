 ...
 * @version
 */
public class caption extends MMLObject {
	...
	public boolean buildStart(String namespaceURI, String localName, String qName, Attributes atts, MMLBuilder builder) {
		if (qName.equals("caption") == true) {
			super.buildStart(namespaceURI,localName,qName,atts,builder);
			printlnStatus(parentElement.getQName() + " " + qName);
			
			/* create tree node */
			caption obj = new caption();
			builder.getMmlTree().addElement( obj );
			obj.setParentIndex( builder.mmlTreeIndex );
			builder.adjustIndex();
			((caption)builder.getElement()).setNamespace( getNamespace() );
			((caption)builder.getElement()).setLocalName( getLocalName() );
			((caption)builder.getElement()).setQName( getQName() );
			((caption)builder.getElement()).setAtts( getAtts() );/* :-) */
			/* atts */
			if (atts != null) {
				for (int i=0; i < atts.getLength(); ++i) {
					if ( ((String)atts.getQName(i)).equals("ID") ) {
						set__ID( atts.getValue(i) );
						((caption)builder.getElement()).set__ID( atts.getValue(i) );
					}
					if ( ((String)atts.getQName(i)).equals("originator") ) {
						set__originator( atts.getValue(i) );
						((caption)builder.getElement()).set__originator( atts.getValue(i) );
					}
					if ( ((String)atts.getQName(i)).equals("confidentiality") ) {
						set__confidentiality( atts.getValue(i) );
						((caption)builder.getElement()).set__confidentiality( atts.getValue(i) );
					}
					if ( ((String)atts.getQName(i)).equals("xml:lang") ) {
						set__xmllang( atts.getValue(i) );
						((caption)builder.getElement()).set__xmllang( atts.getValue(i) );
					}
				}
			}
	...
	public boolean buildEnd(String namespaceURI, String localName, String qName, MMLBuilder builder) {
		if (qName.equals("caption") == true) {
			...
			if (parentElement.getQName().equals("section")) {
				...
				v.addElement( (caption)builder.getElement() );
			}
			...
			if (parentElement.getQName().equals("table")) {
				((table)builder.getParent()).set_caption((caption)builder.getElement());
			}

			if (parentElement.getQName().equals("paragraph")) {
				((paragraph)builder.getParent()).set_caption((caption)builder.getElement());
			}

			if (parentElement.getQName().equals("list")) {
				((list)builder.getParent()).set_caption((caption)builder.getElement());
			}
	...
	public boolean characters(char[] ch, int start, int length, MMLBuilder builder) {
		if (builder.getCurrentElement().getQName().equals("caption")) {
			StringBuffer buffer = new StringBuffer(length);
			buffer.append(ch, start, length);
			vt.addElement(buffer.toString());
			((caption)builder.getElement()).getVt().addElement(buffer.toString());
