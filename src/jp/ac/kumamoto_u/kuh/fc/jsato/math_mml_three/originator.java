 ...
 * @version
 */
public class originator extends MMLObject {
	...
	private originator__type_cd _originator__type_cd = null;
	...
	public void printObject(PrintWriter pw, MMLVisitor visitor) throws IOException {
		if ( this.getQName() != null ) {
			...
			pw.print( ">" );
			/* print content */
			if ( _originator__type_cd != null ) {
				visitor.setIgnoreTab( false );
				pw.print( "\n" );
				_originator__type_cd.printObject(pw, visitor);
			}
	...
	public boolean buildStart(String namespaceURI, String localName, String qName, Attributes atts, MMLBuilder builder) {
		if (qName.equals("originator") == true) {
			...
			builder.adjustIndex();
			((originator)builder.getElement()).setNamespace( getNamespace() );
			((originator)builder.getElement()).setLocalName( getLocalName() );
			((originator)builder.getElement()).setQName( getQName() );
			((originator)builder.getElement()).setAtts( getAtts() );/* :-) */
			/* atts */
			if (atts != null) {
				for (int i=0; i < atts.getLength(); ++i) {
					if ( ((String)atts.getQName(i)).equals("ID") ) {
						set__ID( atts.getValue(i) );
						((originator)builder.getElement()).set__ID( atts.getValue(i) );
					}
					if ( ((String)atts.getQName(i)).equals("HL7-NAME") ) {
						set__HL7_NAME( atts.getValue(i) );
						((originator)builder.getElement()).set__HL7_NAME( atts.getValue(i) );
					}
					if ( ((String)atts.getQName(i)).equals("T") ) {
						set__T( atts.getValue(i) );
						((originator)builder.getElement()).set__T( atts.getValue(i) );
					}
				}
			}
	...
	public boolean buildEnd(String namespaceURI, String localName, String qName, MMLBuilder builder) {
		if (qName.equals("originator") == true) {
			...
			if (parentElement.getQName().equals("clinical_document_header")) {
				Vector v = ((clinical_document_header)builder.getParent()).get_originator();
	...
	public void set_originator__type_cd(originator__type_cd _originator__type_cd) {
		this._originator__type_cd = _originator__type_cd;
	}
	public originator__type_cd get_originator__type_cd() {
		return _originator__type_cd;
	}
