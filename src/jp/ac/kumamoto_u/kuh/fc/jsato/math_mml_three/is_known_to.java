 ...
 * @version
 */
public class is_known_to extends MMLObject {
	...
	public boolean buildStart(String namespaceURI, String localName, String qName, Attributes atts, MMLBuilder builder) {
		if (qName.equals("is_known_to") == true) {
			super.buildStart(namespaceURI,localName,qName,atts,builder);
			printlnStatus(parentElement.getQName() + " " + qName);
			
			/* create tree node */
			is_known_to obj = new is_known_to();
			builder.getMmlTree().addElement( obj );
			obj.setParentIndex( builder.mmlTreeIndex );
			builder.adjustIndex();
			((is_known_to)builder.getElement()).setNamespace( getNamespace() );
			((is_known_to)builder.getElement()).setLocalName( getLocalName() );
			((is_known_to)builder.getElement()).setQName( getQName() );
			((is_known_to)builder.getElement()).setAtts( getAtts() );/* :-) */
			/* atts */
			if (atts != null) {
				for (int i=0; i < atts.getLength(); ++i) {
					if ( ((String)atts.getQName(i)).equals("ID") ) {
						set__ID( atts.getValue(i) );
						((is_known_to)builder.getElement()).set__ID( atts.getValue(i) );
					}
					if ( ((String)atts.getQName(i)).equals("HL7-NAME") ) {
						set__HL7_NAME( atts.getValue(i) );
						((is_known_to)builder.getElement()).set__HL7_NAME( atts.getValue(i) );
					}
					if ( ((String)atts.getQName(i)).equals("T") ) {
						set__T( atts.getValue(i) );
						((is_known_to)builder.getElement()).set__T( atts.getValue(i) );
					}
				}
			}
	...
	public boolean buildEnd(String namespaceURI, String localName, String qName, MMLBuilder builder) {
		if (qName.equals("is_known_to") == true) {
			...
			if (parentElement.getQName().equals("is_known_by")) {
				((is_known_by)builder.getParent()).set_is_known_to((is_known_to)builder.getElement());
			}
