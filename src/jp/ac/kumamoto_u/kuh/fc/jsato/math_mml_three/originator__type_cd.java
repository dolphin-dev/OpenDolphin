 ...
 * @version
 */
public class originator__type_cd extends MMLObject {
	...
	public boolean buildStart(String namespaceURI, String localName, String qName, Attributes atts, MMLBuilder builder) {
		if (qName.equals("originator.type_cd") == true) {
			super.buildStart(namespaceURI,localName,qName,atts,builder);
			printlnStatus(parentElement.getQName() + " " + qName);
			
			/* create tree node */
			originator__type_cd obj = new originator__type_cd();
			builder.getMmlTree().addElement( obj );
			obj.setParentIndex( builder.mmlTreeIndex );
			builder.adjustIndex();
			((originator__type_cd)builder.getElement()).setNamespace( getNamespace() );
			((originator__type_cd)builder.getElement()).setLocalName( getLocalName() );
			((originator__type_cd)builder.getElement()).setQName( getQName() );
			((originator__type_cd)builder.getElement()).setAtts( getAtts() );/* :-) */
			/* atts */
			if (atts != null) {
				for (int i=0; i < atts.getLength(); ++i) {
					if ( ((String)atts.getQName(i)).equals("T") ) {
						set__T( atts.getValue(i) );
						((originator__type_cd)builder.getElement()).set__T( atts.getValue(i) );
					}
					if ( ((String)atts.getQName(i)).equals("V") ) {
						set__V( atts.getValue(i) );
						((originator__type_cd)builder.getElement()).set__V( atts.getValue(i) );
					}
					if ( ((String)atts.getQName(i)).equals("V-T") ) {
						set__V_T( atts.getValue(i) );
						((originator__type_cd)builder.getElement()).set__V_T( atts.getValue(i) );
					}
					if ( ((String)atts.getQName(i)).equals("V-HL7_NAME") ) {
						set__V_HL7_NAME( atts.getValue(i) );
						((originator__type_cd)builder.getElement()).set__V_HL7_NAME( atts.getValue(i) );
					}
					if ( ((String)atts.getQName(i)).equals("DN") ) {
						set__DN( atts.getValue(i) );
						((originator__type_cd)builder.getElement()).set__DN( atts.getValue(i) );
					}
					if ( ((String)atts.getQName(i)).equals("DN-T") ) {
						set__DN_T( atts.getValue(i) );
						((originator__type_cd)builder.getElement()).set__DN_T( atts.getValue(i) );
					}
					if ( ((String)atts.getQName(i)).equals("DN-HL7_NAME") ) {
						set__DN_HL7_NAME( atts.getValue(i) );
						((originator__type_cd)builder.getElement()).set__DN_HL7_NAME( atts.getValue(i) );
					}
					if ( ((String)atts.getQName(i)).equals("ID") ) {
						set__ID( atts.getValue(i) );
						((originator__type_cd)builder.getElement()).set__ID( atts.getValue(i) );
					}
					if ( ((String)atts.getQName(i)).equals("HL7-NAME") ) {
						set__HL7_NAME( atts.getValue(i) );
						((originator__type_cd)builder.getElement()).set__HL7_NAME( atts.getValue(i) );
					}
				}
			}
	...
	public boolean buildEnd(String namespaceURI, String localName, String qName, MMLBuilder builder) {
		if (qName.equals("originator.type_cd") == true) {
			...
			if (parentElement.getQName().equals("originator")) {
				((originator)builder.getParent()).set_originator__type_cd((originator__type_cd)builder.getElement());
			}
