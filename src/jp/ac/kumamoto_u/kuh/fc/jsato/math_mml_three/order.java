 ...
 * @version
 */
public class order extends MMLObject {
	...
	public boolean buildStart(String namespaceURI, String localName, String qName, Attributes atts, MMLBuilder builder) {
		if (qName.equals("order") == true) {
			...
			builder.adjustIndex();
			((order)builder.getElement()).setNamespace( getNamespace() );
			((order)builder.getElement()).setLocalName( getLocalName() );
			((order)builder.getElement()).setQName( getQName() );
			((order)builder.getElement()).setAtts( getAtts() );/* :-) */
			/* atts */
			if (atts != null) {
				for (int i=0; i < atts.getLength(); ++i) {
					if ( ((String)atts.getQName(i)).equals("ID") ) {
						set__ID( atts.getValue(i) );
						((order)builder.getElement()).set__ID( atts.getValue(i) );
					}
					if ( ((String)atts.getQName(i)).equals("HL7-NAME") ) {
						set__HL7_NAME( atts.getValue(i) );
						((order)builder.getElement()).set__HL7_NAME( atts.getValue(i) );
					}
					if ( ((String)atts.getQName(i)).equals("T") ) {
						set__T( atts.getValue(i) );
						((order)builder.getElement()).set__T( atts.getValue(i) );
					}
				}
			}
	...
	public boolean buildEnd(String namespaceURI, String localName, String qName, MMLBuilder builder) {
		if (qName.equals("order") == true) {
			...
			if (parentElement.getQName().equals("fulfills_order")) {
				Vector v = ((fulfills_order)builder.getParent()).get_order();
