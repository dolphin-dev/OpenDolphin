 ...
 * @version
 */
public class mmlSmplan extends MMLObject {
	...
	public boolean buildStart(String namespaceURI, String localName, String qName, Attributes atts, MMLBuilder builder) {
		if (qName.equals("mmlSm:plan") == true) {
			super.buildStart(namespaceURI,localName,qName,atts,builder);
			printlnStatus(parentElement.getQName() + " " + qName);
			
			/* create tree node */
			mmlSmplan obj = new mmlSmplan();
			builder.getMmlTree().addElement( obj );
			obj.setParentIndex( builder.mmlTreeIndex );
			builder.adjustIndex();
			((mmlSmplan)builder.getElement()).setNamespace( getNamespace() );
			((mmlSmplan)builder.getElement()).setLocalName( getLocalName() );
			((mmlSmplan)builder.getElement()).setQName( getQName() );
			((mmlSmplan)builder.getElement()).setAtts( getAtts() );/* :-) */
			/* atts */
	...
	public boolean buildEnd(String namespaceURI, String localName, String qName, MMLBuilder builder) {
		if (qName.equals("mmlSm:plan") == true) {
			...
			if (parentElement.getQName().equals("mmlSm:SummaryModule")) {
				((mmlSmSummaryModule)builder.getParent()).set_plan((mmlSmplan)builder.getElement());
			}
	...
	public boolean characters(char[] ch, int start, int length, MMLBuilder builder) {
		if (builder.getCurrentElement().getQName().equals("mmlSm:plan")) {
			StringBuffer buffer = new StringBuffer(length);
			buffer.append(ch, start, length);
			vt.addElement(buffer.toString());
			((mmlSmplan)builder.getElement()).getVt().addElement(buffer.toString());
