			...
			if (atts != null) {
				for (int i=0; i < atts.getLength(); ++i) {
					if ( ((String)atts.getQName(i)).equals("mmlRp:wardCode") ) {
						...
						((mmlRppWard)builder.getElement()).set__mmlRpwardCode( atts.getValue(i) );
					}
					if ( ((String)atts.getQName(i)).equals("mmlRp:wardCodeId") ) {
						set__mmlRpwardCodeId( atts.getValue(i) );
						((mmlRppWard)builder.getElement()).set__mmlRpwardCodeId( atts.getValue(i) );
					}
				}
			}
	...
	public boolean buildEnd(String namespaceURI, String localName, String qName, MMLBuilder builder) {
		if (qName.equals("mmlRp:pWard") == true) {
			...
			if (parentElement.getQName().equals("mmlRp:perform")) {
				((mmlRpperform)builder.getParent()).set_pWard((mmlRppWard)builder.getElement());
			}
