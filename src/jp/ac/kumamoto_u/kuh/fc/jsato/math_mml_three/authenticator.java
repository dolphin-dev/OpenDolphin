 ...
 * @version
 */
public class authenticator extends MMLObject {
	...
	private authenticator__type_cd _authenticator__type_cd = null;
	...
	public void printObject(PrintWriter pw, MMLVisitor visitor) throws IOException {
		if ( this.getQName() != null ) {
			...
			pw.print( ">" );
			/* print content */
			if ( _authenticator__type_cd != null ) {
				visitor.setIgnoreTab( false );
				pw.print( "\n" );
				_authenticator__type_cd.printObject(pw, visitor);
			}
	...
	public boolean buildStart(String namespaceURI, String localName, String qName, Attributes atts, MMLBuilder builder) {
		if (qName.equals("authenticator") == true) {
			...
			builder.adjustIndex();
			((authenticator)builder.getElement()).setNamespace( getNamespace() );
			((authenticator)builder.getElement()).setLocalName( getLocalName() );
			((authenticator)builder.getElement()).setQName( getQName() );
			((authenticator)builder.getElement()).setAtts( getAtts() );/* :-) */
			/* atts */
			if (atts != null) {
				for (int i=0; i < atts.getLength(); ++i) {
					if ( ((String)atts.getQName(i)).equals("ID") ) {
						set__ID( atts.getValue(i) );
						((authenticator)builder.getElement()).set__ID( atts.getValue(i) );
					}
					if ( ((String)atts.getQName(i)).equals("HL7-NAME") ) {
						set__HL7_NAME( atts.getValue(i) );
						((authenticator)builder.getElement()).set__HL7_NAME( atts.getValue(i) );
					}
					if ( ((String)atts.getQName(i)).equals("T") ) {
						set__T( atts.getValue(i) );
						((authenticator)builder.getElement()).set__T( atts.getValue(i) );
					}
				}
			}
	...
	public boolean buildEnd(String namespaceURI, String localName, String qName, MMLBuilder builder) {
		if (qName.equals("authenticator") == true) {
			...
			if (parentElement.getQName().equals("clinical_document_header")) {
				Vector v = ((clinical_document_header)builder.getParent()).get_authenticator();
	...
	public void set_authenticator__type_cd(authenticator__type_cd _authenticator__type_cd) {
		this._authenticator__type_cd = _authenticator__type_cd;
	}
	public authenticator__type_cd get_authenticator__type_cd() {
		return _authenticator__type_cd;
	}
