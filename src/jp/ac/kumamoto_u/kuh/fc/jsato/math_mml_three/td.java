 ...
 * @version
 */
public class td extends MMLObject {
	...
	public boolean buildStart(String namespaceURI, String localName, String qName, Attributes atts, MMLBuilder builder) {
		if (qName.equals("td") == true) {
			...
			builder.adjustIndex();
			((td)builder.getElement()).setNamespace( getNamespace() );
			((td)builder.getElement()).setLocalName( getLocalName() );
			((td)builder.getElement()).setQName( getQName() );
			((td)builder.getElement()).setAtts( getAtts() );/* :-) */
			/* atts */
			if (atts != null) {
				for (int i=0; i < atts.getLength(); ++i) {
					if ( ((String)atts.getQName(i)).equals("ID") ) {
						set__ID( atts.getValue(i) );
						((td)builder.getElement()).set__ID( atts.getValue(i) );
					}
					if ( ((String)atts.getQName(i)).equals("originator") ) {
						set__originator( atts.getValue(i) );
						((td)builder.getElement()).set__originator( atts.getValue(i) );
					}
					if ( ((String)atts.getQName(i)).equals("confidentiality") ) {
						set__confidentiality( atts.getValue(i) );
						((td)builder.getElement()).set__confidentiality( atts.getValue(i) );
					}
					if ( ((String)atts.getQName(i)).equals("xml:lang") ) {
						set__xmllang( atts.getValue(i) );
						((td)builder.getElement()).set__xmllang( atts.getValue(i) );
					}
					if ( ((String)atts.getQName(i)).equals("abbr") ) {
						set__abbr( atts.getValue(i) );
						((td)builder.getElement()).set__abbr( atts.getValue(i) );
					}
					if ( ((String)atts.getQName(i)).equals("axis") ) {
						set__axis( atts.getValue(i) );
						((td)builder.getElement()).set__axis( atts.getValue(i) );
					}
					if ( ((String)atts.getQName(i)).equals("headers") ) {
						set__headers( atts.getValue(i) );
						((td)builder.getElement()).set__headers( atts.getValue(i) );
					}
				}
			}
	...
	public boolean buildEnd(String namespaceURI, String localName, String qName, MMLBuilder builder) {
		if (qName.equals("td") == true) {
			...
			if (parentElement.getQName().equals("tr")) {
				...
				v.addElement( (td)builder.getElement() );
			}
	...
	public boolean characters(char[] ch, int start, int length, MMLBuilder builder) {
		if (builder.getCurrentElement().getQName().equals("td")) {
			StringBuffer buffer = new StringBuffer(length);
			buffer.append(ch, start, length);
			vt.addElement(buffer.toString());
			((td)builder.getElement()).getVt().addElement(buffer.toString());
