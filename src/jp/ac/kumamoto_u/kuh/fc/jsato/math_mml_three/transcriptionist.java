 ...
 * @version
 */
public class transcriptionist extends MMLObject {
	...
	public void printObject(PrintWriter pw, MMLVisitor visitor) throws IOException {
		if ( this.getQName() != null ) {
			...
			pw.print( ">" );
			/* print content */
			if ( _transcriptionist__type_cd != null ) {
				visitor.setIgnoreTab( false );
				pw.print( "\n" );
				_transcriptionist__type_cd.printObject(pw, visitor);
			}
	...
	public boolean buildStart(String namespaceURI, String localName, String qName, Attributes atts, MMLBuilder builder) {
		if (qName.equals("transcriptionist") == true) {
			super.buildStart(namespaceURI,localName,qName,atts,builder);
			printlnStatus(parentElement.getQName() + " " + qName);
			
			/* create tree node */
			transcriptionist obj = new transcriptionist();
			builder.getMmlTree().addElement( obj );
			obj.setParentIndex( builder.mmlTreeIndex );
			builder.adjustIndex();
			((transcriptionist)builder.getElement()).setNamespace( getNamespace() );
			((transcriptionist)builder.getElement()).setLocalName( getLocalName() );
			((transcriptionist)builder.getElement()).setQName( getQName() );
			((transcriptionist)builder.getElement()).setAtts( getAtts() );/* :-) */
			/* atts */
			if (atts != null) {
				for (int i=0; i < atts.getLength(); ++i) {
					if ( ((String)atts.getQName(i)).equals("ID") ) {
						set__ID( atts.getValue(i) );
						((transcriptionist)builder.getElement()).set__ID( atts.getValue(i) );
					}
					if ( ((String)atts.getQName(i)).equals("HL7-NAME") ) {
						set__HL7_NAME( atts.getValue(i) );
						((transcriptionist)builder.getElement()).set__HL7_NAME( atts.getValue(i) );
					}
					if ( ((String)atts.getQName(i)).equals("T") ) {
						set__T( atts.getValue(i) );
						((transcriptionist)builder.getElement()).set__T( atts.getValue(i) );
					}
				}
			}
	...
	public boolean buildEnd(String namespaceURI, String localName, String qName, MMLBuilder builder) {
		if (qName.equals("transcriptionist") == true) {
			...
			if (parentElement.getQName().equals("clinical_document_header")) {
				((clinical_document_header)builder.getParent()).set_transcriptionist((transcriptionist)builder.getElement());
			}
	...
	public void set_transcriptionist__type_cd(transcriptionist__type_cd _transcriptionist__type_cd) {
		this._transcriptionist__type_cd = _transcriptionist__type_cd;
	}
	public transcriptionist__type_cd get_transcriptionist__type_cd() {
		return _transcriptionist__type_cd;
	}
