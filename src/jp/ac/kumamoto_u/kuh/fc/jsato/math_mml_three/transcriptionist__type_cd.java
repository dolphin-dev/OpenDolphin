/**
 *
 * transcriptionist__type_cd.java
 ...
 * @version
 */
public class transcriptionist__type_cd extends MMLObject {
	...
	public transcriptionist__type_cd() {
	}
	...
	public boolean buildStart(String namespaceURI, String localName, String qName, Attributes atts, MMLBuilder builder) {
		if (qName.equals("transcriptionist.type_cd") == true) {
			super.buildStart(namespaceURI,localName,qName,atts,builder);
			printlnStatus(parentElement.getQName() + " " + qName);
			
			/* create tree node */
			transcriptionist__type_cd obj = new transcriptionist__type_cd();
			builder.getMmlTree().addElement( obj );
			obj.setParentIndex( builder.mmlTreeIndex );
			builder.adjustIndex();
			((transcriptionist__type_cd)builder.getElement()).setNamespace( getNamespace() );
			((transcriptionist__type_cd)builder.getElement()).setLocalName( getLocalName() );
			((transcriptionist__type_cd)builder.getElement()).setQName( getQName() );
			((transcriptionist__type_cd)builder.getElement()).setAtts( getAtts() );/* :-) */
			/* atts */
			if (atts != null) {
				for (int i=0; i < atts.getLength(); ++i) {
					if ( ((String)atts.getQName(i)).equals("T") ) {
						set__T( atts.getValue(i) );
						((transcriptionist__type_cd)builder.getElement()).set__T( atts.getValue(i) );
					}
					if ( ((String)atts.getQName(i)).equals("V") ) {
						set__V( atts.getValue(i) );
						((transcriptionist__type_cd)builder.getElement()).set__V( atts.getValue(i) );
					}
					if ( ((String)atts.getQName(i)).equals("V-T") ) {
						set__V_T( atts.getValue(i) );
						((transcriptionist__type_cd)builder.getElement()).set__V_T( atts.getValue(i) );
					}
					if ( ((String)atts.getQName(i)).equals("V-HL7_NAME") ) {
						set__V_HL7_NAME( atts.getValue(i) );
						((transcriptionist__type_cd)builder.getElement()).set__V_HL7_NAME( atts.getValue(i) );
					}
					if ( ((String)atts.getQName(i)).equals("DN") ) {
						set__DN( atts.getValue(i) );
						((transcriptionist__type_cd)builder.getElement()).set__DN( atts.getValue(i) );
					}
					if ( ((String)atts.getQName(i)).equals("DN-T") ) {
						set__DN_T( atts.getValue(i) );
						((transcriptionist__type_cd)builder.getElement()).set__DN_T( atts.getValue(i) );
					}
					if ( ((String)atts.getQName(i)).equals("DN-HL7_NAME") ) {
						set__DN_HL7_NAME( atts.getValue(i) );
						((transcriptionist__type_cd)builder.getElement()).set__DN_HL7_NAME( atts.getValue(i) );
					}
					if ( ((String)atts.getQName(i)).equals("ID") ) {
						set__ID( atts.getValue(i) );
						((transcriptionist__type_cd)builder.getElement()).set__ID( atts.getValue(i) );
					}
					if ( ((String)atts.getQName(i)).equals("HL7-NAME") ) {
						set__HL7_NAME( atts.getValue(i) );
						((transcriptionist__type_cd)builder.getElement()).set__HL7_NAME( atts.getValue(i) );
					}
				}
			}
	...
	public boolean buildEnd(String namespaceURI, String localName, String qName, MMLBuilder builder) {
		if (qName.equals("transcriptionist.type_cd") == true) {
			...
			if (parentElement.getQName().equals("transcriptionist")) {
				((transcriptionist)builder.getParent()).set_transcriptionist__type_cd((transcriptionist__type_cd)builder.getElement());
			}
