 ...
 * @version
 */
public class thead extends MMLObject {
	...
	public boolean buildStart(String namespaceURI, String localName, String qName, Attributes atts, MMLBuilder builder) {
		if (qName.equals("thead") == true) {
			...
			builder.adjustIndex();
			((thead)builder.getElement()).setNamespace( getNamespace() );
			((thead)builder.getElement()).setLocalName( getLocalName() );
			((thead)builder.getElement()).setQName( getQName() );
			((thead)builder.getElement()).setAtts( getAtts() );/* :-) */
			/* atts */
			if (atts != null) {
				for (int i=0; i < atts.getLength(); ++i) {
					if ( ((String)atts.getQName(i)).equals("ID") ) {
						set__ID( atts.getValue(i) );
						((thead)builder.getElement()).set__ID( atts.getValue(i) );
					}
					if ( ((String)atts.getQName(i)).equals("originator") ) {
						set__originator( atts.getValue(i) );
						((thead)builder.getElement()).set__originator( atts.getValue(i) );
					}
					if ( ((String)atts.getQName(i)).equals("confidentiality") ) {
						set__confidentiality( atts.getValue(i) );
						((thead)builder.getElement()).set__confidentiality( atts.getValue(i) );
					}
					if ( ((String)atts.getQName(i)).equals("xml:lang") ) {
						set__xmllang( atts.getValue(i) );
						((thead)builder.getElement()).set__xmllang( atts.getValue(i) );
					}
					if ( ((String)atts.getQName(i)).equals("align") ) {
						set__align( atts.getValue(i) );
						((thead)builder.getElement()).set__align( atts.getValue(i) );
					}
					if ( ((String)atts.getQName(i)).equals("char") ) {
						set__char( atts.getValue(i) );
						((thead)builder.getElement()).set__char( atts.getValue(i) );
					}
					if ( ((String)atts.getQName(i)).equals("charoff") ) {
						set__charoff( atts.getValue(i) );
						((thead)builder.getElement()).set__charoff( atts.getValue(i) );
					}
					if ( ((String)atts.getQName(i)).equals("valign") ) {
						set__valign( atts.getValue(i) );
						((thead)builder.getElement()).set__valign( atts.getValue(i) );
					}
				}
			}
	...
	public boolean buildEnd(String namespaceURI, String localName, String qName, MMLBuilder builder) {
		if (qName.equals("thead") == true) {
			...
			if (parentElement.getQName().equals("table")) {
				((table)builder.getParent()).set_thead((thead)builder.getElement());
			}
	...
	public boolean characters(char[] ch, int start, int length, MMLBuilder builder) {
		if (builder.getCurrentElement().getQName().equals("thead")) {
			StringBuffer buffer = new StringBuffer(length);
			buffer.append(ch, start, length);
			vt.addElement(buffer.toString());
			((thead)builder.getElement()).getVt().addElement(buffer.toString());
