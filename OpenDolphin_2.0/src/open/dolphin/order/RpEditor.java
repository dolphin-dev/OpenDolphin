...
public final class RpEditor extends AbstractStampEditor {
    ...
    private static final String[] RP_CODE = {"211", "212", "221", "222", "231", "232"};
    
    private static final String[] BUNDLE_MEMO = {"内用（院内処方）", "内用（院外処方）", "頓用（院内処方）", "頓用（院外処方）", "外用（院内処方）", "外用（院外処方）"};
    ...
    public Object getValue() {
        ...
        if (view.getTonyoChk().isSelected()) {
            naiGaiTon = 220;     // 内容=210,頓用=220, 外用=230
        } else {
            naiGaiTon = 0;
        }
        ...
        tableModel = new ListTableModel<MasterItem>(COLUMN_NAMES, START_NUM_ROWS, METHOD_NAMES, null) {
            ...
            public boolean isCellEditable(int row, int col) {
                ...
                if (col == ONEDAY_COLUMN) {
                    ...
                    return (code==null || editableComment || codeIsAdmin) ? false : true;
                }
