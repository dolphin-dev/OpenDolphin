    ...
    private void loadStampTree() {
        ...
        final SimpleWorker worker = new SimpleWorker<List<IStampTreeModel>, Void>() {
            ...
            protected void failed(Throwable e) {
                String fatalMsg = "スタンプツリーの読み込みに失敗しました。\nアプリケーションを起動できません。";
            ...
            protected void timeout() {
                String fatalMsg = "スタンプツリーの読み込み中にタイムアウトになりました。\nアプリケーションを起動できません。";
                ClientContext.getBootLogger().fatal(fatalMsg);
                JOptionPane.showMessageDialog(null, fatalMsg, ClientContext.getFrameTitle("初期化"), JOptionPane.WARNING_MESSAGE);
                System.exit(1);
            }
...
//    }
    ...
    /**
     * 更新の対象となる JAR ファイルをリストアップする。
     * @return UpdateObject のリスト
     */
    private List<UpdateObject> listUpdateTargets() {
        ...
        // JAR ファイルの名前がキー、lastModified が属性の値
        if (prop != null) {
